...
# Descifrar el token en tiempo de ejecución utilizando AES-256-GCM
def get_github_token():
    ...
    GITHUB_TOKEN_CIFRADO = os.getenv("GITHUB_TOKEN_CIFRADO")
    ...
    # Decodificar NONCE, TAG y el token cifrado
    nonce = base64.b64decode(NONCE)
    tag = base64.b64decode(TAG)
    token_cifrado = base64.b64decode(GITHUB_TOKEN_CIFRADO)

    # Descifrar el token utilizando AES-256-GCM
    cipher = AES.new(password_32bytes, AES.MODE_GCM, nonce=nonce)
    token = cipher.decrypt_and_verify(token_cifrado, tag).decode()
    return token
